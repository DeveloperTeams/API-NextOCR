...
class DocumentSegmenter:
    ...
    DEFAULT_MODEL_PATH = "/tmp/u2net.onnx"
    INPUT_SIZE = 320
    MIN_MODEL_SIZE_BYTES = 10 * 1024 * 1024

    def __init__(self, model_path: Optional[str] = None):
        self.model_url = os.getenv("U2NET_MODEL_URL", self.MODEL_URL)
        self.model_path = self._get_default_model_path(model_path)
        self.session: Optional[ort.InferenceSession] = None
        self.input_size: Tuple[int, int] = (self.INPUT_SIZE, self.INPUT_SIZE)
        self._load_model()

    def _get_default_model_path(self, model_path: Optional[str] = None) -> str:
        path = model_path or os.getenv("U2NET_MODEL_PATH") or self.DEFAULT_MODEL_PATH
    ...
    def _download_model(self, target_path: str) -> None:
        ...
        try:
            ...
            if file_size < self.MIN_MODEL_SIZE_BYTES:
                raise RuntimeError(
                    f"model download appears incomplete (size={file_size} bytes)"
                )
    ...
    def _load_model(self) -> None:
        ...
        use_cuda = os.getenv("ONNXRUNTIME_USE_CUDA", "0").strip().lower() in {
            "1",
            "true",
            "yes",
        }
        if use_cuda and "CUDAExecutionProvider" in available:
            ...
            logger.warning(
                "ONNXRUNTIME_USE_CUDA is enabled but CUDAExecutionProvider is unavailable; using CPU"
            )
        ...
        for attempt in range(2):
            try:
                ...
                message = str(e)
                is_corrupt = (
                    "INVALID_PROTOBUF" in message.upper()
                    or "PROTOBUF" in message.upper()
                )

                if attempt == 0 and is_corrupt:
                    logger.warning(
                        "U²-Net model file appears corrupted, re-downloading model"
                    )
                    try:
                        ...
                        logger.error(
                            f"Failed to re-download U²-Net model: {download_error}"
                        )
    ...
    def _refine_mask_edges(
        self, image: np.ndarray, mask: np.ndarray, iterations: int = 3
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            w, h = (
                min(image.shape[1] - x, w + 2 * margin),
                min(image.shape[0] - y, h + 2 * margin),
            )
    ...
    def extract_corners_from_mask(
        ...
        if cv2.contourArea(doc_contour) < image_area * min_area_ratio:
            logger.debug(
                f"Document contour too small: {cv2.contourArea(doc_contour)}/{image_area}"
            )
            return None
    ...
    def _validate_quadrilateral(
        self, points: List[Tuple[float, float]], image_shape: Tuple[int, int]
    ) -> bool:
        """
        ...
        width_top = np.sqrt(
            (ordered[1][0] - ordered[0][0]) ** 2 + (ordered[1][1] - ordered[0][1]) ** 2
        )
        width_bottom = np.sqrt(
            (ordered[2][0] - ordered[3][0]) ** 2 + (ordered[2][1] - ordered[3][1]) ** 2
        )
        height_left = np.sqrt(
            (ordered[3][0] - ordered[0][0]) ** 2 + (ordered[3][1] - ordered[0][1]) ** 2
        )
        height_right = np.sqrt(
            (ordered[2][0] - ordered[1][0]) ** 2 + (ordered[2][1] - ordered[1][1]) ** 2
        )
    ...
    def detect(
        ...
        mask = self.get_document_mask(
            image, adaptive_threshold=adaptive_threshold, refine_edges=refine_edges
        )
    ...
    def _compute_confidence(
        ...
        height_diff = abs(height_left - height_right) / max(
